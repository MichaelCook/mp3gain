...
void init_layer3(int down_sample_sblimit)
{
    ...
    for (i = 0; i < 18; i++)
    {
        win[0][i]    = win[1][i]    = 0.5 * sin(M_PI / 72.0 * (double)(2 * (i + 0) + 1)) / cos(M_PI * (double)(2 *
                                      (i + 0) + 19) / 72.0);
        win[0][i + 18] = win[3][i + 18] = 0.5 * sin(M_PI / 72.0 * (double)(2 * (i + 18) + 1)) / cos(M_PI * (double)(2 *
                                          (i + 18) + 19) / 72.0);
    }
    for (i = 0; i < 6; i++)
    {
    ...
    for (gr = 0; gr < 2; gr++)
    {
        for (ch = 0; ch < stereo; ch++)
        {
            ...
            }
            {
                ...
                if ((unsigned char)qss > *maxGain)
                {
                    *maxGain = (unsigned char)qss;
                }
                if ((unsigned char)qss < *minGain)
                {
