...
int ReadMP3ID3v1Tag(FILE *fi, unsigned char **tagbuff, long *tag_offset)
{
    ...
    if (fseek(fi, *tag_offset - 128, SEEK_SET))
    {
        return 0;
    }
    if (fread(tmp, 1, 128, fi) != 128)
    {
        return 0;
    }
    if (memcmp(tmp, "TAG", 3))
                               ...
                               long *tag_offset)
{
    ...
    if (fseek(fp, *tag_offset - 128, SEEK_SET))
    {
        return 0;
    }
    ...
    *id3tagbuff = (unsigned char *)malloc(128);
    ...
    if (memcmp(T.ID, "LYRICS200", sizeof(T.ID)))
    {
        return 0;
    }
...
static unsigned long Read_LE_Uint32(const char *p)
{
...
int ReadMP3APETag(FILE *fp,  struct MP3GainTagInfo *info, struct APETagStruct **apeTag, long *tag_offset)
{
    ...
    if (fseek(fp, *tag_offset - sizeof(T), SEEK_SET))
    {
        return 0;
    }
    if (fread(&T, 1, sizeof(T), fp) != sizeof(T))
    ...
    if (memcmp(T.ID, "APETAGEX", sizeof(T.ID)))
    {
        return 0;
    }
    ...
    if ((TagLen = Read_LE_Uint32(T.Length)) < sizeof(T))
    {
        return 0;
    }
    if (fseek(fp, *tag_offset - TagLen, SEEK_SET))
    {
        return 0;
    }
    ...
    *apeTag = (struct APETagStruct *)malloc(sizeof(struct APETagStruct));
    ...
    for (p = buff; p < end && TagCount--;)
    {
        ...
        p += 4;
        Read_LE_Uint32(p);      /* flags */
        p += 4;

        remaining = (unsigned long)(end - p);
        isize = strlen_max(p, remaining);
        if (isize >= remaining || vsize > MAX_FIELD_SIZE || isize + 1 + vsize > remaining)
        {
            break;
        }
...
int truncate_file(const char *filename, long truncLength)
{
    int fd;
                       ...
                       int saveTimeStamp)
{
    ...
    if (info->haveMinMaxGain)
    {
        ...
        mp3gainTagData += 15;
        sprintf(mp3gainTagData, "%03d",
                info->minGain); /* write directly to tag buffer, because we'll replace the trailing null char anyhow */
        mp3gainTagData[3] = ',';
        mp3gainTagData += 4;
    ...
    if (info->haveAlbumMinMaxGain)
    {
        ...
        mp3gainTagData += 21;
        sprintf(mp3gainTagData, "%03d",
                info->albumMinGain); /* write directly to tag buffer, because we'll replace the trailing null char anyhow */
        mp3gainTagData[3] = ',';
        mp3gainTagData += 4;
    ...
    if (info->haveUndo)
    {
        ...
        mp3gainTagData += 13;
        sprintf(mp3gainTagData, "%+04d",
                info->undoLeft); /* write directly to tag buffer, because we'll replace the trailing null char anyhow */
        mp3gainTagData[4] = ',';
        mp3gainTagData += 5;
        sprintf(mp3gainTagData, "%+04d",
                info->undoRight); /* write directly to tag buffer, because we'll replace the trailing null char anyhow */
        mp3gainTagData[4] = ',';
        mp3gainTagData += 5;
        if (info->undoWrap)
        {
...
int RemoveMP3GainAPETag(const char *filename, int saveTimeStamp)
{
    ...
    if (info.haveAlbumGain || info.haveAlbumPeak || info.haveTrackGain || info.haveTrackPeak || info.haveMinMaxGain ||
        info.haveAlbumMinMaxGain || info.haveUndo)
    {
        info.dirty = !0;
    }
